...
namespace Viture.XR
{
    ...
    public sealed class VitureHandSubsystem : XRHandSubsystem
    {
        ...
        protected override void OnStart()
        {
            ...
            VitureXR.HandTracking.aimSensitivity = VitureSettings.GetOrCreate().HandAimSensitivity;
        ...
        {
            private readonly float[] m_RawHandData = new float[VitureHandTrackingData.k_HandDataLength];
            private readonly Vector3[] m_JointPositions = new Vector3[VitureHandTrackingData.k_HandJointCount];
            private readonly Quaternion[] m_JointRotations = new Quaternion[VitureHandTrackingData.k_HandJointCount];
            ...
            {
                s_CurrentAimSensitivity = sensitivity;
            }
            ...
            {
                int baseIndex = handedness == Handedness.Left ? 0 : VitureHandTrackingData.k_HandDataLength / 2;
                ...
                for (int i = 0; i < VitureHandTrackingData.k_HandJointCount; i++)
                {
                ...
                foreach (var mapping in VitureHandTrackingData.s_JointMapping)
                {
            ...
            private void UpdateHandDevice(Handedness handedness, bool isTracked)
            {
                ...
                var palmFacing = m_RawHandData[baseIndex + VitureHandTrackingData.k_PalmFacingOffset];
                var gesture = m_RawHandData[baseIndex + VitureHandTrackingData.k_GestureOffset];
                ...
                Vector3 currentArmDirection = Vector3.forward;
                if (m_MainCamera != null)
                {
                    ...
                    currentArmDirection = (palmPosition - shoulderPosition).normalized;
                }
                
                // Hand coordinate
                var indexPosition = m_JointPositions[(int)VitureHandJointID.IndexProximal];
                var middlePosition = m_JointPositions[(int)VitureHandJointID.MiddleProximal];
                var littlePosition = m_JointPositions[(int)VitureHandJointID.LittleProximal];
                
                var rawForward = (middlePosition - wristPosition).normalized;
                var rawRight = (littlePosition - indexPosition).normalized;

                Vector3 stableUp;
                Vector3 stableForward;
                
                if (handedness == Handedness.Left)
                {
                    stableUp = Vector3.Cross(rawRight, rawForward).normalized;
                    stableForward = Vector3.Cross(stableUp, rawRight).normalized;
                }
                else
                {
                    stableUp = Vector3.Cross(rawForward, rawRight).normalized;
                    stableForward = Vector3.Cross(rawRight, stableUp).normalized;
                }
